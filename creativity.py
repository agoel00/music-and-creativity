...
class Model:
    ...
    def dat(self, words, minimum=5):
        ...
        if len(uniques) >= minimum:
            ...
            return np.nan # Not enough valid words
    ...
    def rat(self, words, minimum=5):
        """Compute RAT score"""
        self.rubric = {
            'cream/skate/water': 'ice',
            'loser/throat/spot': 'sore',
            'preserve/ranger/forest': 'forest',
        ...
        similarities = []
        for ground_truth, response in zip(self.rubric.values(), subset):
            sim = 1 - self.distance(ground_truth, response)
